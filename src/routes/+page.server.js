...
export async function load({ locals }) {
    ...
    const { data: listedestaches, error: tacheError } = await supabase
        .from("liste_tache")
        .select("id_tache, nom_tache, user_id, status")
        .eq("user_id", userId);
    ...
    // Récupérer les sites
    const { data: listedessites, error: siteError } = await supabase
        .from("liste_site")
        .select("*")
        .eq("user_id", userId);

    const date_actuel = new Date();
    const date = date_actuel.getDate();
...
export const actions = {
    createitem: async ({ request, locals }) => {
        ...
        await supabase
            .from("liste_tache")
            .insert([{ nom_tache: nomtache, user_id: locals.user.id, status: 0 }]);
    },
